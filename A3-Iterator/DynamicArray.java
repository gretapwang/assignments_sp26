...
public class DynamicArray<T> implements ListADT<T>, Iterable<T> {
    ...
    /**
     * Overloaded constructor creates an empty DynamicArray with zero initial capacity.
     */
    public DynamicArray() {
        this(0);
    }
    ...
    /**
     * Iterator class for a DynamicArray. 
     * 
     * @author Greta Wang
     * @version Spring 2026
     */
    private class DynamicArrayIterator implements Iterator<T> {
        ...
        private int index; // Index which the iterator is positioned before
    ...
    /**
     * Returns the tail of the list, starting at the specified index, as a new list.
     ...
     * Returns the tail as a new list.
     * 
     * @param index The starting index for the tail
     * @return The copy of the tail
     * @throws IndexOutOfBoundsException For index less than 0 or greater than size
    ...
    public DynamicArray<T> splitTransfer(int index) {
        DynamicArray<T> tail = this.splitCopy(index);
        for (int i = this.size() - 1; i >= index; i--) {
            this.remove(i);
        }
        return tail;
